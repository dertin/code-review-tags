...
async function restoreOptions() {
  ...
  populateDefaultLabelDropdown(items.labels || DEFAULTS.labels, items.defaultLabel || DEFAULTS.defaultLabel);
}

async function saveOptions() {
  const customLabels = el.labels.value.split(',').map(s => s.trim()).filter(Boolean);
  const customDecorations = el.decorations.value.split(',').map(s => s.trim()).filter(Boolean);

  await api.storage.sync.set({
    labels: customLabels.length ? customLabels : DEFAULTS.labels,
    decorations: customDecorations.length ? customDecorations : DEFAULTS.decorations,
